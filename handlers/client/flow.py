...
def proceed_to_location(chat_id: str, message_id: int):
    ...
    if not service_id:
        ...
        send_safe(
            bot,
            chat_id,
            f"{Icons.ERROR} Error: sesión expirada. Usá /start."
        )
        return
...
def handle_client_confirmation(call):
    ...
    workers, status, extra = worker_jobs.find_available_workers(
        service_id,
        lat,
        lon,
        hora
    ...
    if not workers:
        ...
        send_safe(
            bot,
            chat_id,
            f"{Icons.WARNING} No hay profesionales disponibles."
        )
    ...
    if not result:
        ...
        send_safe(
            bot,
            chat_id,
            f"{Icons.ERROR} El profesional ya no está disponible."
        )
...
""",
    ...
    edit_safe(
        ...
        f"{Icons.SEARCH} Buscando profesionales..."
    )
...
def handle_client_accept_worker(call):
    ...
    notify_client(
        worker_id,
        f"Cliente aceptó el servicio #{request_id}"
    )

    send_safe(
        bot,
        chat_id,
        f"{Icons.SUCCESS} Solicitud confirmada."
    )
