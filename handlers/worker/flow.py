...
def save_worker_data(chat_id: str, dni: str):
    ...
    # Guardar info principal del trabajador
    db_execute("""
        INSERT OR REPLACE INTO workers (chat_id, nombre, telefono, dni, disponible, lat, lon, last_update)
    ...
    for service_id in selected_services:
        ...
        precio_db = None if precio is None else float(precio)
        db_execute(
            "INSERT OR REPLACE INTO worker_services (chat_id, service_id, precio) VALUES (?, ?, ?)",
            (str(chat_id), service_id, precio_db),
            commit=True
    )
