...
def get_session(chat_id: str):
    try:
        ...
        logger.error(f"[SESSION ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        return {"state": "IDLE", "data": {}}
...
def set_state(chat_id: str, state: str, data: dict = None):
    try:
        ...
        logger.info(f"[STATE SET] chat_id={chat_id} -> {state}")
    except Exception as e:
        logger.error(f"[SET STATE ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
...
def update_data(chat_id: str, **kwargs):
    try:
        ...
        logger.info(f"[DATA UPDATE] chat_id={chat_id} -> {kwargs}")
    except Exception as e:
        logger.error(f"[UPDATE DATA ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
...
def get_data(chat_id: str, key: str, default=None):
    try:
        ...
        logger.error(f"[GET DATA ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        return default

def clear_state(chat_id: str):
    try:
        db_execute("DELETE FROM sessions WHERE chat_id = ?", (str(chat_id),), commit=True)
        logger.info(f"[STATE CLEAR] chat_id={chat_id}")
    except Exception as e:
        logger.error(f"[CLEAR STATE ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
...
def handle_worker_start(message):
    ...
    try:
        ...
        logger.error(f"[START ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
...
def ask_next_price(chat_id: str):
    try:
        ...
        logger.error(f"[ASK PRICE ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error pidiendo precio. Reiniciá flujo.")
...
def handle_price_input(message):
    ...
    try:
        ...
        logger.error(f"[PRICE INPUT ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error procesando precio. Reiniciá flujo.")
...
def ask_worker_name(chat_id: str):
    try:
        ...
        logger.error(f"[ASK NAME ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error pidiendo nombre.")
...
def handle_name_input(message):
    ...
    try:
        ...
        logger.error(f"[NAME INPUT ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error procesando nombre.")
...
def ask_worker_phone(chat_id: str):
    try:
        ...
        logger.error(f"[ASK PHONE ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error pidiendo teléfono.")
...
def handle_phone_input(message):
    ...
    try:
        ...
        logger.error(f"[PHONE INPUT ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error procesando teléfono.")
...
def ask_worker_dni(chat_id: str):
    try:
        ...
        logger.error(f"[ASK DNI ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error pidiendo DNI.")
...
def handle_dni_input(message):
    ...
    try:
        ...
        logger.error(f"[DNI INPUT ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error procesando DNI.")
...
def ask_worker_location(chat_id: str):
    try:
        ...
        logger.error(f"[ASK LOCATION ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error pidiendo ubicación.")
...
def handle_location(message):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[SHOW WORKER MENU ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        clear_state(chat_id)
    except Exception as e:
        logger.error(f"[LOCATION ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Error procesando ubicación.")
...
def cancel_flow(chat_id: str):
    try:
        ...
        logger.error(f"[CANCEL FLOW ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
...
def save_worker_data(chat_id: str, dni: str):
    try:
        ...
        # Guardar información principal del trabajador
        db_execute("""
            INSERT OR REPLACE INTO workers 
            (chat_id, nombre, telefono, dni, disponible, lat, lon, last_update)
        ...
        # Guardar servicios seleccionados y sus precios
        ...
        logger.info(f"[WORKER SAVED] chat_id={chat_id}, servicios={selected_services}, precios={prices}")
    except Exception as e:
        logger.error(f"[SAVE WORKER ERROR] chat_id={chat_id} -> {traceback.format_exc()}")
        bot.send_message(chat_id, f"{Icons.ERROR} Ocurrió un error guardando tus datos.")
        clear_state(chat_id)
